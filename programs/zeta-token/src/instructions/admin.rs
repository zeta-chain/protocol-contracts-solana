use crate::contexts::{Initialize, UpdateTssAddress};
...
/// Update TSS address.
pub fn update_tss_address(ctx: Context<UpdateTssAddress>, new_tss_address: [u8; 20]) -> Result<()> {
    let zeta_token_pda = &mut ctx.accounts.zeta_token_pda;
